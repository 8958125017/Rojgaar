...
mapAddressForEvent:any;
callMethodForEvent(){
  var mapAddress=localStorage.getItem("lattlngt");
  this.mapAddressForEvent=mapAddress;
  this.Googlemap();
}
...
Googlemap(){
  ...
  }else if(this.mapAddressForEvent !=undefined && this.mapAddressForEvent !=''){
    var MapEvntAdd=this.mapAddressForEvent;
    var latitE=MapEvntAdd.split(',');
    var locLatE=latitE[0];
    var locLngE=latitE[1];
    this.uluru = { lat: parseFloat(locLatE), lng: parseFloat(locLngE) };
  }
  else{
